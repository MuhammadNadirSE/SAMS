...
namespace TMD.Web
{
    public class Utility
    {
        ...
        public static DateTime ConvertTimeByGMT(int GMT)
        {
            if(GMT < 0)
                GMT *= (-1);
            var ClientDateTime = DateTime.Now.AddHours(GMT);
            return ClientDateTime;
        }
        
        public static DateTime ConvertTimeByGMT(int GMT, DateTime ClientTime)
        {
            if(GMT < 0)
                GMT *= (-1);
            var ClientDateTime = ClientTime.AddHours(GMT);
            return ClientDateTime;
        }
        public static DateTime SubtractTimeByGMT(int GMT, DateTime ClientTime)
        {
            if (GMT < 0)
                GMT *= (-1);
            var ClientDateTime = ClientTime.AddHours(-GMT);
